...
/**
 * M5.8 document access governance (pure, no database).
 *
 * Documents reuse the composed pipeline (consent + ReBAC + break-glass). Uploading
 * is a NON-encounter write (the `upload` action does not trigger the ABAC encounter
 * constraint) — a patient may upload an insurance card between visits. A read is
 * what gates the storage pointer. This asserts read/upload composition, the
 * no-encounter-required property, and consent revocation propagation.
 */
...
function context(overrides: Partial<DocumentAccessContext> = {}): DocumentAccessContext {
  return {
    decisionRequestId: "doc-authz-1",
    actorId: PATIENT,
    actorRole: "patient",
    actorType: "patient",
    patientId: PATIENT,
...
describe("M5.8 document access (composed pipeline, non-encounter upload)", () => {
  it("denies a read with no access-control state (default-deny)", () => {
    const decision = decideDocumentAccessFrom(context(), "read", NO_INPUTS);
    expect(decision.status).toBe("denied");
    expect(decision.reasonCode).toBe("consent-missing");
  });

  it("allows a patient to read their own documents with granted consent", () => {
    const decision = decideDocumentAccessFrom(context(), "read", {
  ...
  it("allows a patient to upload with consent and NO active encounter", () => {
    const decision = decideDocumentAccessFrom(context({ activeEncounter: false }), "upload", {
      consent: grantedConsent(),
      relationship: null,
      breakGlass: null
    });
  ...
  it("allows a guardian read with consent AND an active relationship", () => {
    const decision = decideDocumentAccessFrom(
      context({
        actorId: "guardian-1",
        actorRole: "guardian",
        actorType: "guardian",
  ...
  it("allows an emergency document read via break-glass (no consent)", () => {
    const decision = decideDocumentAccessFrom(
  ...
  it("propagates a consent revocation to the very next read decision", () => {
    const ctx = context();
    expect(
      decideDocumentAccessFrom(ctx, "read", {
        consent: grantedConsent(),
        relationship: null,
        breakGlass: null
      }).status
    ...
    const afterRevoke = decideDocumentAccessFrom(ctx, "read", {
