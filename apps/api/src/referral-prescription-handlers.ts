...
export function transitionReferralStatus(input: ReferralStatusUpdateInput): ReferralDraftAdvanced {
  ...
  return {
    ...
    sentAt: input.toStatus === "sent" ? input.updatedAt : input.referral.sentAt,
    respondedAt: ["accepted", "declined"].includes(input.toStatus) ? input.updatedAt : input.referral.respondedAt,
    completedAt: input.toStatus === "completed" ? input.updatedAt : input.referral.completedAt,
    cancelledAt: input.toStatus === "cancelled" ? input.updatedAt : input.referral.cancelledAt,
...
export function transitionPrescriptionStatus(
  ...
  return {
    ...input.prescription,
    status: input.toStatus,
    verifiedAt: input.toStatus === "verified" ? input.updatedAt : input.prescription.verifiedAt,
    dispensedAt: input.toStatus === "dispensed" ? input.updatedAt : input.prescription.dispensedAt,
    completedAt: input.toStatus === "completed" ? input.updatedAt : input.prescription.completedAt,
    cancelledAt: input.toStatus === "cancelled" ? input.updatedAt : input.prescription.cancelledAt
  };
}
