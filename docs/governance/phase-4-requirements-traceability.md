...
| 3. Granular consent with full domain coverage, versioning, and revocation | PARTIAL-FOUNDATION-DELIVERED | Domain-scoped, version-aware, revocable consent model and ABAC policy checks are scaffolded with deterministic tests; production persistence and operational workflows remain for later increments. |
...
| P04-REQ-003 | Consent is granular, revocable, and version-aware in decision scaffolds | Implementation map Phase 4 | apps/api/src/granular-consent.ts | apps/api/src/granular-consent.test.ts, apps/api/src/authorization-policy-handlers.ts, apps/api/src/authorization-policy-handlers.test.ts | COMPLETED-GRANULAR-VERSIONED-CONSENT-SCAFFOLD |
...
- API unit tests command: `node node_modules/vitest/vitest.mjs run apps/api/src/audit-event-workflows.test.ts apps/api/src/audit-event-workflows.integration.test.ts apps/api/src/runtime-routes.test.ts`
- API unit tests result: `3 passed files, 21 passed tests (0.97s)`
- Browser tests command: `node node_modules/@playwright/test/cli.js test -c playwright.step4.config.ts`
- Browser tests result: `15 passed (15.0s)`
