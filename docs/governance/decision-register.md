...
| DEC-P05-MKT-006 | Ship both light and dark themes from the first marketing release. Every semantic color token has both light and dark values; `[data-theme="dark"]` selector toggles the palette; defaults to `prefers-color-scheme`. Persisted in `localStorage` under `nh-theme`. Contrast tests cover both themes. | APPROVED | Design owner | docs/exec-plans/P05-MKT-001-marketing-visual-language-and-tokens.md; docs/exec-plans/P05-MKT-002-marketing-component-library.md |
| DEC-P05-MKT-007 | Use Google Fonts CDN to load Fraunces (display) and Atkinson Hyperlegible (body) via `<link>` in `apps/patient-web/app/layout.tsx`. CSP restricts font origins to `fonts.googleapis.com` and `fonts.gstatic.com` only. **RESIDUAL PRIVACY CAVEAT**: Google Fonts CDN receives the visitor's IP address on every page load. This creates a coherence gap between the site's privacy copy and the plumbing. Privacy Owner review is required before pilot. If reversed, migration to `next/font/google` (self-hosted at build) is a one-issue change. | APPROVED-WITH-PRIVACY-CAVEAT | Product owner (chose Google Fonts CDN over the recommended self-hosted `next/font/google` option); Privacy Owner review pending before pilot | docs/exec-plans/P05-MKT-007-marketing-plumbing-seo-error-and-consent.md |
| DEC-P05-MKT-008 | Authorize a P05-MKT-007 sub-track to close world-class plumbing gaps: Google Fonts CDN wiring, per-page SEO metadata (`generateMetadata` + `marketing-seo` family), `sitemap.xml` + `robots.txt`, schema.org structured data (Organization / MedicalOrganization / WebSite / WebPage / FAQPage / BreadcrumbList), marketing-safe 404 / 500 / global-error pages, dormant cookie-consent shell (env-gated), and Web Vitals budgets (LCP, CLS, INP, FCP, TBT) on desktop and mobile. | APPROVED | Product owner | docs/exec-plans/P05-MKT-007-marketing-plumbing-seo-error-and-consent.md |
| DEC-P05-MKT-009 | P05-ISS-003 primitive set extended to include Table and Timeline (called out in Phase 5 implementation map). Table needed for pricing/comparison; Timeline needed for storytelling and later clinical history. | APPROVED | Design/Engineering owner | docs/exec-plans/P05-ISS-003-design-system-primitives-expansion.md |
