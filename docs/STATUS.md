...
- P05-MKT-003: COMPLETED 2026-07-13, marketing content registry populated with 296 draft entries across 27 families covering PILOT pages (home, how-it-works, patients, family-diaspora, doctors, pharmacies, laboratories, trust-and-safety, privacy-overview, accessibility, faq, contact, legal, emergency), non-PILOT scope-caveat pages (employers, hmos, hospitals, home-care), auth chrome (signin, create-account, forgot-password, reset-password), per-page SEO metadata, marketing-safe error pages, cookie-consent shell copy, shared microcopy, and 12 CTA labels aligned to approved CTA-IDs (CTA-CREATE-ACCOUNT, CTA-SIGN-IN, CTA-CONTACT, CTA-FAQ, CTA-CALL-LOCAL-HELP, CTA-CONTACT-SUPPORT, CTA-BACK, CTA-LEARN-MORE, CTA-BOOK-WALKTHROUGH, CTA-GET-STARTED, CTA-VIEW-PRICING, CTA-DOWNLOAD-APP). Added `public-site` surface + release-policy approval gate; voice-tone lint (banned-claim list, generic error, emoji-in-clinical, brand casing) runs over every marketing entry and is wired into `pnpm content:validate`.
- P05-MKT-004: COMPLETED 2026-07-14, 15 PILOT marketing pages (home, how-it-works, patients, family-plans, diaspora, doctors, pharmacies, laboratories, trust-and-safety, privacy-overview, accessibility, faq, contact, legal-and-regulatory-notices, emergency) + 4 auth chrome pages (sign-in, create-account, forgot-password, reset-password) ship under `apps/patient-web/app/*/page.tsx`. Root layout wraps every route in `ContentProvider(marketingContentEntries)` + `EmergencyRibbon` + `SiteHeader` + `SiteFooter`. Every page exports `generateMetadata` sourcing `marketing-seo.<slug>.*`. `batch1-production-page.tsx`, `batch2-production-page.tsx`, and their route catalogs deleted. 13 301 redirects in `next.config.mjs` cover the /for-* rename map, /login→/sign-in, /register→/create-account, /help→/contact, /patient→/patients, /trust-safety→/trust-and-safety. Content registry grew to 319 entries with 21 nav + footer microcopy additions. Vitest 5 files / 64 tests passing (content-registry regression + inline-string boundary + redirect boundary). Playwright p05-mkt-004 config covers desktop / tablet / mobile / desktop-a11y / mobile-a11y / reduced-motion projects.
...
- Next action: execute P05-MKT-005 (non-PILOT scope-caveat pages) — ship /employers, /hmos, /hospitals-and-referrals, /home-care using the existing non-PILOT segment entries + their `scope.caveat` copy; the /for-* → new-slug redirects are already in place. P05-ISS-002 browser-observable low-bandwidth UX extension continues in parallel.
