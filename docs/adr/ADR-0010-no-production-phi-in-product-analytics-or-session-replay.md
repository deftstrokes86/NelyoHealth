# ADR-0010: No production PHI in product analytics or session replay
...
ACCEPTED-IN-PRINCIPLE; IMPLEMENTATION-PENDING
...
General product analytics and session replay must not receive production protected clinical information. Session replay is disabled by default on sensitive surfaces. Restricted clinical-quality analytics uses a separate approved pathway.
...
Full analytics event payloads; masked session replay everywhere; no analytics; minimized product analytics plus restricted clinical-quality analytics.
...
Session replay proposal; new analytics vendor; new clinical-quality use; employer or HMO reporting; AI/model training; new cross-border processor.
...
General product analytics and session replay must not receive full clinical notes, full prescriptions, full laboratory results, diagnoses where not explicitly approved, authentication secrets, payment credentials, patient addresses, provider coordinates, pre-payment provider details, patient-provider location pairs, break-glass reason text, or guardian dispute evidence.

## Restricted Clinical-Quality Analytics Boundary

Restricted clinical-quality analytics requires a separate approved pathway, clinical governance, privacy approval, minimum necessary data, restricted access, audit, retention controls, and no general product-analytics access.
