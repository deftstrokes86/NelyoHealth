# ADR-0007: Payer and clinical-access separation
...
Funding and clinical access are separate policy decisions. A payer, sponsor, family administrator, employer, HMO, finance operator, or claims operator receives only purpose-specific access.
...
Payer receives full care record; family administrator inherits member access; employer receives individual clinical reporting; HMO receives unrestricted clinical history; separate access evaluation.
...
New payer, sponsor, HMO, employer, claims, guardian, delegation, or support-access model.
