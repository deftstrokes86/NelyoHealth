# ADR-0009: Video platform decision deferred
...
DEFERRED; EVALUATION-REQUIRED-BEFORE-VIDEO-IMPLEMENTATION
...
Do not yet choose between managed video provider, self-hosted WebRTC platform, managed deployment of an open-source video platform, or hybrid model.
...
Managed video provider; self-hosted WebRTC; managed open-source platform; hybrid model.
...
Before consultation video implementation begins.
...
The video-platform evaluation must cover clinical reliability, reconnection, audio fallback, mobile-browser support, accessibility, data flows, recording disabled by default, session metadata, cross-border processing, data residency, encryption, credential and token model, incident cooperation, operational burden, cost, vendor lock-in, exit strategy, observability, network quality, and Nigerian connectivity conditions.

## Deferred Selection Boundary

No managed provider, self-hosted platform, managed open-source deployment, or hybrid model is selected by this ADR. Any selection requires Phase 1 evaluation and domain approval before video consultation implementation begins.
